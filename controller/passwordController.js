...
module.exports.sendForgotPasswordLink = asyncHandler(async (req, res) => {
  ...
  // use findOne  email: req.body.email  (to check if user inside database)
  if (!user) {
    return res.status(404).json({ message: "user not found" });
  }

  const secret = process.env.JWT_SECRET_KEY + user.password;
  // Change secrect key to change also user.password--and secret will be changed
  // to let any old Token not valied
  const token = jwt.sign({ email: user.email, id: user.id }, secret, {
    expiresIn: "10m",
  });

  const link = `http://localhost:5000/password/reset-password/${user._id}/${token}`;
  //console.log("Here is the real link: ", link);
...
module.exports.resetThePassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    //Hashing password before saveing inside Database
